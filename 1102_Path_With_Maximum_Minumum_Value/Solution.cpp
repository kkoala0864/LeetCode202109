...
bool dfs(const vector<vector<int>>& grid, int mid, vector<vector<int>>& flag, int x, int y) {
	...
	for (const auto& ai : axis) {
		...
		if (i < 0 || i >= grid.size() || j < 0 || j >= grid[0].size() || grid[i][j] < mid || flag[i][j]) continue;
		flag[i][j] = true;
		if (dfs(grid, mid, flag, i, j)) return true;
	}
	return false;
}
...
int Solution::maximumMinimumPath(vector<vector<int>>& grid) {
	...
	int mid = 0;
	while (left <= right) {
		for (auto& iter : flag) fill(iter.begin(), iter.end(), false);
		mid = (left + right) / 2;

		if (dfs(grid, mid, flag, 0, 0)) left = mid + 1;
		else right = mid - 1;
	}
	return right;
}
