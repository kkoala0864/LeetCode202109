	...
	for (int i = 0 ; i < m ; ++i) {
		for (int j = 0 ; j < n ; ++j) {
			...
			for (const auto& pattern : patterns) {
				if (pattern.size() != tmp.size()) continue;
				int i = 0;
				while (i < pattern.size()) {
					if (pattern[i].first == tmp[i].first && pattern[i].second == tmp[i].second) ++i;
					else break;;
				}
			...
			if (!find) patterns.emplace_back(tmp);
		}
	}
...
void buildAndCompare(vector<vector<int>>& grid, int x, int y, int i, int j, Trie*& node) {
	...
	if (!node->child[4]) node->child[4] = new Trie();
	node = node->child[4];
}
