...
vector<int> Solution::sortItems(int n, int m, vector<int>& group, vector<vector<int>>& beforeItems) {
	...
	queue<int> iq, gq;
	for (int i = 0 ; i < n ; ++i) if (ibCnt[i] == 0) iq.emplace(i);
	while (!iq.empty()) {
		int cur = iq.front();
		iq.pop();
		...
		for (const auto& ni : ib[cur]) {
			--ibCnt[ni];
			if (ibCnt[ni] == 0) iq.emplace(ni);
		}
	}
	for (int i = 0 ; i < m ; ++i) {
		cout << i << " | ";
		for (const auto& j : r[i]) cout << j << " ";
		cout << endl;
	}

	for (int i = 0 ; i < m ; ++i) if (gbCnt[i] == 0) gq.emplace(i);
	while (!gq.empty()) {
		int cur = gq.front();
		gq.pop();
		for (const auto& v : r[cur]) result.emplace_back(v);
		for (const auto& ng : gb[cur]) {
			--gbCnt[ng];
			if (gbCnt[ng] == 0) gq.emplace(ng);
		}
	}
