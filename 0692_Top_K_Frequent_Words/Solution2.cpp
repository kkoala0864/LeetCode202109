...
vector<string> Solution::topKFrequent2(vector<string>& words, int k) {
	...
	for (const auto& w : words) ++freqCnt[w];

	vector<TrieNode*> freqTrie(words.size() + 1, nullptr);

	for (const auto& fc : freqCnt) {
		if (!freqTrie[fc.second]) freqTrie[fc.second] = new TrieNode();
	...
	string local;
	for (int i = freqTrie.size() - 1 ; i >= 1 ; --i) getWord(freqTrie[i], k, local, result);
	return result;
}
