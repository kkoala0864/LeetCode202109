...
vector<vector<int>> sum2(vector<int> &nums, int target, int idx) {
	...
	while (start < end) {
		...
		if ((val < target) || (start > idx && nums[start] == nums[start - 1]))
			++start;
		else if ((val > target) || (end < nums.size() - 1 && nums[end] == nums[end + 1]))
			--end;
		else
...
vector<vector<int>> ksum(vector<int> &nums, int target, int idx, int size) {
	...
	for (int i = idx; i < (nums.size() - size + 1); ++i) {
		...
		while (i < (nums.size() - size + 1) && nums[i] == nums[i + 1])
			++i;
	}
	return result;
}
