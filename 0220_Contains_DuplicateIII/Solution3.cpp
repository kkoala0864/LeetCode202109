...
bool Solution::containsNearbyAlmostDuplicate3(vector<int>& nums, int k, int t) {
	...
	for (int i = 0 ; i < nums.size() ; ++i) {
		...
		if (nums[i] < 0) --bucketIdx;

		if (uMap.find(bucketIdx) != uMap.end()) return true;
		if (uMap.find(bucketIdx+1) != uMap.end() && abs(nums[i] - uMap[bucketIdx+1]) <= t) return true;
		if (uMap.find(bucketIdx-1) != uMap.end() && abs(nums[i] - uMap[bucketIdx-1]) <= t) return true;
		uMap[bucketIdx] = nums[i];

		if (uMap.size() > k) {
			long removeIdx = (long)nums[i - k] / base;
			if (nums[i-k] < 0) --removeIdx;
