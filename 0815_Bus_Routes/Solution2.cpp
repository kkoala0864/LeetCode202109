...
int Solution::numBusesToDestination2(vector<vector<int>>& routes, int source, int target) {
	...
	for (const auto& r : numToRoute[target]) targetRoutes.emplace(r);
	...
	while (!que.empty()) {
		...
		if (targetRoutes.count(curRoute)) return routeShortestPath[curRoute];

		for (const auto& num : routes[curRoute]) {
			for (const auto& r : numToRoute[num]) {
				if (routeShortestPath[r] != INT_MAX) continue;
