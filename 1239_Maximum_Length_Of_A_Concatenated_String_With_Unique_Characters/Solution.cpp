...
void dfs(vector<string>& arr, int idx, int bitmask, int size, int& result) {
	...
	for (int i = idx ; i < arr.size() ; ++i) {
		...
		for (; ci < arr[i].size() ; ++ci) {
			if (nextBitMask >> (arr[i][ci]-'a') & 1) break;
			else nextBitMask |= 1 << (arr[i][ci]-'a');
		}
		if (ci != arr[i].size()) dfs(arr, i + 1, bitmask, size, result);
		else dfs(arr, i + 1, nextBitMask, size + arr[i].size(), result);
	}
}
