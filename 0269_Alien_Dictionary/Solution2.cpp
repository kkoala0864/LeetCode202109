...
string Solution::alienOrder2(vector<string>& words) {
	...
	for (int i = 0 ; i < words.size() ; ++i) {
		for (const auto& c : words[i]) us.emplace(c-'a');
		for (int j = i + 1 ; j < words.size() ; ++j) {
			int size = min(words[i].size(), words[j].size());
			if (words[i].size() > words[j].size() && words[i].substr(0, size) == words[j]) return "";
			for (int k = 0 ; k < size ; ++k) {
				if (words[i][k] == words[j][k]) continue;
				if (od[words[i][k]-'a'].count(words[j][k]-'a')) break;
	...
	string result;
	for (int i = 0 ; i < 26 ; ++i) {
		if (us.count(i) == 0) continue;
		if (icnt[i] == 0) que.emplace(i);
	}
