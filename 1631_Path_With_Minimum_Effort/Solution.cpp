...
int Solution::minimumEffortPath(vector<vector<int>> &heights) {
	...
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < n; ++j) {
			if (j < n - 1)
				diff[abs(heights[i][j] - heights[i][j + 1])].push_back({i * n + j, i * n + j + 1});
			if (i < m - 1)
				diff[abs(heights[i][j] - heights[i + 1][j])].push_back({i * n + j, (i + 1) * n + j});
			unionFind[i][j] = i * n + j;
		}
	}
	...
	for (const auto &iter : diff) {
		...
		for (const auto &edge : iter.second) {
			...
			if (first < second)
				unionFind[second / n][second % n] = first;
			else
				unionFind[first / n][first % n] = second;
			if (find(m * n - 1, n, unionFind) == 0)
				return result;
		}
	}
	return result;
