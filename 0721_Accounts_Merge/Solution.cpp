...
vector<vector<string>> Solution::accountsMerge(vector<vector<string>>& accounts) {
	...
	for (const auto& account : accounts) {
		...
		if (graph[name].find(root) == graph[name].end()) graph[name][root] = root;
		else find(root, graph[name]);

		for (int i = 2 ; i < account.size() ; ++i) {
			string node = account[i];
			if (graph[name].find(node) == graph[name].end()) graph[name][node] = root;
			else {
	...
	for (const auto& ele : uMap) {
		...
		for (const auto& mail : ele.second) {
			...
			for (const auto& m : mail.second) insert.emplace_back(m);
