...
int Solution::maxRectangleArea(vector<vector<int>> &points) {
	...
	for (int i = 0; i < points.size(); ++i) {
		for (int j = i + 1; j < points.size(); ++j) {
			if (points[j][0] <= points[i][0] || points[j][1] <= points[i][1])
				continue;
			if (xa[points[j][0]].count(points[i][1]) == 0 || xa[points[i][0]].count(points[j][1]) == 0)
				continue;
	...
	for (const auto &c : candidate) {
		...
		for (; i < points.size(); ++i) {
			if (i == c[1] || i == c[2])
				continue;
			if ((points[i][0] == x1 && points[i][1] == y2) || (points[i][0] == x2 && points[i][1] == y1))
				continue;
			if ((x1 <= points[i][0] && points[i][0] <= x2) && (y1 <= points[i][1] && points[i][1] <= y2))
				break;
		}
		if (i < points.size())
			continue;
